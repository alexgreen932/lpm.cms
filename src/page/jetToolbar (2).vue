<template>
  <div v-if="ops.editable" class="form-toolbar wc-x" :class="[cls, part]">
    <div class="d-bg">
      ...
      <div v-if="notFirst(index)" class="d-wrap">
        <i class="fa-solid" :class="prev" v-tt:top-center-small="$__('Move +')" @click="moveItem(index, index - 1)"></i>
      </div>
      <div v-if="notLast(index)" class="d-wrap">
        <i class="fa-solid" :class="next" v-tt:top-center-small="$__('Move -')" @click="moveItem(index, index + 1)"></i>
      </div>
      <div v-if="cls == 'section' && notPart()" class="d-wrap">
        <i class="fa-solid fa-floppy-disk" v-tt:top-center-small="$__('Save as Pattern')" @click="saveAsPattern()"></i>
      </div>
...
export default {
  props: ["elements", "index", "page_index", "cls", "sec", "dir", "type", "part"],//rin type
  data() {
    return {
      ops,
      isAdmin: true,
